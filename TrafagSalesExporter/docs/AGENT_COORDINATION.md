...
| Claude | Vertrieb / Marktumfrage in die App holen, Excel abloesen | **RESERVIERT ab 2026-08-13:** `Models/MarketSurveyEntry.cs` (neu), `Services/MarketSurveyPageService.cs` (neu), `Components/Pages/MarketSegments.razor`, Schema-/Seed-Services, `Services/UiTextGeneratedTranslations.cs`, Tests, `.tmp_tools/ImportMarketSurvey/**` (neu) | In Arbeit. Ziel: `Railway_MarketSurvey_TSC_2026_05.xlsx` wird in der Anwendung pflegbar, damit die Datei entfallen kann. Additiv, keine Aenderung an bestehenden Berechnungen |
