...
| Claude | Spanien / Buchungsdatum in den Sage-Export aufnehmen (ISS-004.2) | `SageSpainExportPackage/SageSpainFinalExportPackage/Export-SageSpainSalesCsv.ps1`, `SageSpainExportPackage/SageSpainFinalExportPackage/Run-SpainRangeExportAndUpload-AllInOne.ps1`, `SageSpainExportPackage/SageSpainFinalExportPackage/README.txt`, `SageSpainExportPackage/SageSpainFinalExportPackage/SageSpainFinalExportPackage.zip`, `scripts/Export-SageSpainSalesCsv.ps1` (byte-identische Spiegelung) | Abgeschlossen am 2026-08-17. `FacturasTB.FechaAsiento` als `PostingDate` und `FacturasTB.Asiento` als `PostingDocument` in beide Export-Queries aufgenommen, damit Ingo das Paket auf den spanischen Sage-Server kopieren kann. Bewusst `OUTER APPLY` mit `TOP 1` statt `JOIN`: im Schemaauszug haben 70 von 3'642 Rechnungsschluesseln mehrere `FacturasTB`-Zeilen, bei 6 davon mit abweichendem `FechaAsiento`, ein `JOIN` haette die Verkaufszeilen vervielfacht und den spanischen Umsatz erhoeht. Nachweis: alle vier erzeugten SQL-Varianten mit `Microsoft.SqlServer.TransactSql.ScriptDom` als gueltiges T-SQL geparst, Gegenprobe mit kaputtem SQL schlaegt fehl; alle drei Skripte parsen als PowerShell. NEBENBEFUND: die eingecheckte `SageSpainFinalExportPackage.zip` war seit dem rclone-Fix veraltet, ihr fehlte `Resolve-RcloneExecutable`; das Neubauen zieht diesen Fix mit. KEIN Anwendungscode, KEINE Datenbank, KEIN Deploy, keine Tests noetig. Der Join-Schluessel ist bis zur Live-Pruefung in Spanien eine ANNAHME und im Skript wie im README als solche gekennzeichnet. Reservierung frei |
