...
Die Root-Dateien `AGENTS.md` und `CLAUDE.md` sowie `router.md` machen
...
| Codex | Finance / CH-AT Eigenfertigungspruefung | `.tmp_tools/CheckChAtOrigin/**`, `.tmp_tools/CheckChAtCosts/**`, `.tmp_tools/BuildSupplierReport/**`, `docs/FINANCE_SUPPLIER.md`, `docs/Supplier_Laenderstatus_CH_AT_Pruefung_2026-08-11.docx` | Abgeschlossen; nur read-only Analyse und neue Nachweisdokumente, keine Produktivdaten- oder Anwendungsaenderung |
| Codex | Finance / Supplier-Fallback CH-Werkstamm | `Models/GroupMaterialMaster.cs`, `Services/SapGatewayPlantMaterialReader.cs`, Supplier-Klassifikation, Settings/Schema/Excel/Cockpit/Tests, `docs/FINANCE_SUPPLIER.md` | Produktiv deployed; Modus ChPlantMaster und 66.049 MARC-1100-Materialien nach Neustart bestaetigt |
| Codex | Finance / Andreas-Nachtrag lokale Standardkosten | Supplier-Klassifikation/-Rechnung, Management-Cockpit, Excel-Hilfe, Tests, `docs/FINANCE_STANDARDKOSTEN.md` | Produktiv deployed am 2026-08-12 10:23 durch Claude nach Freigabe durch Ingo; 478/478 Tests gruen, Wirkung nachgemessen |
...
| Claude | Finance / Deploy des Andreas-Nachtrags und Offene-Punkte-Liste | `.tmp_tools/DeployAndreasLocal/**` (neu), `docs/rag/DEPLOYMENT.md`, `lastchange.md`, `docs/rag/FINANCE.md`, `docs/FINANCE_STANDARDKOSTEN.md`, `docs/Issue_Log_Konsolidiert_2026-08-12.tsv`, `docs/FINANCE_OFFENE_PUNKTE_2026-08-12.md` (neu), `docs/Issue_Log_Konsolidiert_2026-08-12.tsv` (neu), `router.md` | Deploy abgeschlossen und dokumentiert; kein Anwendungscode geaendert, nur Dokumentation. Reservierung frei |
...
| Claude | Marktsegmente / Jahresfilter, 3D-Analyse und ueberlagerte Filterbeschriftungen | `Services/MarketSegmentPageService.cs`, `Components/Pages/MarketSegments.razor`, `Components/MarketSurveyPanel.razor`, `Components/Pages/ManagementCockpit.razor` (nur zwei `MudSelect`-Zeilen im Finance-Pivot), `TrafagSalesExporter.Tests/MarketSegmentPageServiceTests.cs`, `Services/UiTextGeneratedTranslations.cs` (nur neue Texte), `docs/MARKTSEGMENTE_RAILWAY_2026-08-13.md` | Abgeschlossen und PRODUKTIV deployed am 2026-08-14 21:02, Commit `7419473`, ohne Alarm. Umgesetzt: Jahresfilter fuer die ganze Seite mit derselben Datumsregel wie das zentrale Excel, Jahresspalte in der Ergebnissicht, drehbare 3D-Analyse im Ergebnisreiter auf Basis der vorhandenen Engine `wwwroot/js/finance3d.js` ohne neue Bibliothek, und die Korrektur der ueber ihrer Beschriftung liegenden Auswahlwerte. Zusaetzlich in `Components/Layout/MainLayout.razor` behoben: `pa-4` auf `MudMainContent` ueberschrieb per `!important` den Abstand zur festen Kopfleiste, wodurch die obersten rund 48 Pixel JEDER Seite unsichtbar waren; jetzt `px-4 pb-4`. Kein Schemawechsel, `CustomerMarketSegments` unveraendert, Excel-Export und `MarketSegmentResolver` nicht angefasst. `520/520` Tests gruen. Angemeldet lokal gegen `trafag_exporter.db` sichtgeprueft, drei Testzuordnungen danach wieder entfernt. Deploy mit Vorher-Sicherung `trafag_exporter.db.before-segment-year-20260814-205358.bak`, DLL bitgleich, sechs Routen HTTPS 200, keine Migration. Offen bleibt nur der angemeldete produktive Sichtprueflauf. Details: `docs/MARKTSEGMENTE_RAILWAY_2026-08-13.md` Abschnitt 13 und `docs/rag/DEPLOYMENT.md`. Reservierung frei |
| Claude | Spanien / Buchungsdatum in den Sage-Export aufnehmen (ISS-004.2) | `SageSpainExportPackage/SageSpainFinalExportPackage/Export-SageSpainSalesCsv.ps1`, `SageSpainExportPackage/SageSpainFinalExportPackage/Run-SpainRangeExportAndUpload-AllInOne.ps1`, `SageSpainExportPackage/SageSpainFinalExportPackage/README.txt`, `SageSpainExportPackage/SageSpainFinalExportPackage/SageSpainFinalExportPackage.zip`, `scripts/Export-SageSpainSalesCsv.ps1` (byte-identische Spiegelung), neu `SageSpainExportPackage/SageSpainFinalExportPackage/Analyze-SpainPostingDateKey.ps1` | Abgeschlossen am 2026-08-17, inklusive Live-Verifikation. `FacturasTB.FechaAsiento` als `PostingDate` und `FacturasTB.Asiento` als `PostingDocument` in beide Export-Queries aufgenommen. Bewusst `OUTER APPLY` mit `TOP 1` statt `JOIN`, Begruendung siehe Historie unten. NACHTRAG Live-Session mit Ingo per RDP auf dem spanischen Sage-Server: neues read-only Diagnosewerkzeug `Analyze-SpainPostingDateKey.ps1` gebaut. Ergebnis: Schluessel `CodigoEmpresa`/`Ejercicio`/`Serie`/`Factura` ist KORREKT, 53 von 53 Treffern (100%) auf einem bereits gebuchten Fenster (23.-30.07.). Die anfaenglich leeren `PostingDate`-Werte (58 von 58 Zeilen im Testlauf 10.-13.08.) lagen an Buchungsverzug (letztes gebuchtes `FechaFactura` war 2026-07-30), nicht am Schluessel. Zusaetzlich gefundener und behobener Bug in `Run-SpainRangeExportAndUpload-AllInOne.ps1`: `Resolve-RcloneExecutable` nutzte `$MyInvocation.MyCommand.Path` innerhalb einer Funktion (dort zuverlaessig `$null`), jetzt `$PSScriptRoot`. Zeitfenster des taeglichen Delta-Laufs von 7 auf 35 Tage erweitert, damit Rechnungen mit ~2-3 Wochen Buchungsverzug nicht dauerhaft ohne `PostingDate` bleiben; laut `docs/rag/MANUAL_IMPORT.md` dedupliziert die App ueber `SourceLineId`, daher unproblematisch. Einmaliger Nachtrag Januar-Mai 2026 lokal exportiert (1'571 Zeilen, `PostingDate` 100% gefuellt) und per `rclone copy` nach `Import/Finance/Spanien` hochgeladen. Mailtext an Santi Gomez vorbereitet (NICHT von Claude versendet), er soll die alte 7-Tage-Version auf dem Server durch die neue 35-Tage-Version ersetzen. KEIN Anwendungscode, KEINE Datenbank, KEIN Deploy, keine Tests noetig. Details: `docs/FINANCE_ES_BUCHUNGSDATUM_2026-08-03.md` Abschnitt 9. Offen: Santi muss die Datei serverseitig ersetzen; danach PostingDate-Spaltenzuordnung im Seed fuer Spanien manuell in den Einstellungen setzen, Reimport, Jahresverteilung TRES neu messen. Reservierung frei |
| Claude | Projektmanagement / Verdichtung des Chatprotokolls | `projektmanagement/PROJEKTSTATUS.md` (neu), `projektmanagement/kontext.txt` (nur UEBERHOLT-Kopf ergaenzt) | Abgeschlossen am 2026-08-14: `kontext.txt` mit 2025 Zeilen ChatGPT-Protokoll zu `PROJEKTSTATUS.md` verdichtet, sechs offene Arbeitspakete PM-01 bis PM-06 und 21 erledigte Punkte. Beim Abgleich gegen das Repository korrigiert: PM-04 ist seit 2026-08-12 erledigt und PM-01 hat den Transport als echten Blocker, beides war im Protokoll falsch als „Klaerung offen" gefuehrt. Nachgetragen: ZC12-Codeanalyse als PM-02 und das PPWR-Paket als PM-06 mit Verweis auf die Codex-Dokumentation. Ebenfalls gepflegt: `router.md` und `lastchange.md`. Kein Anwendungscode, keine Produktivdaten, kein Build, kein Deploy. Reservierung frei |
| Claude | Doku / Repo-weite Markdown-Aufraeumung und neue Router-Hierarchie | `router.md` und `baum.md` (neu, Repo-Wurzel), `docs/router/*.md` (neu), `AGENTS.md`, `CLAUDE.md`, 41 geloeschte und 11 neue Dateien unter `docs/`, Verweisanpassung in allen aktiven `.md` | Abgeschlossen am 2026-08-17. **Neue Navigation:** `router.md` ist der einzige globale Einstieg mit Vorrangregeln und sieben Themenaesten, je Ast ein Unterrouter unter `docs/router/`, `baum.md` als Vollstaendigkeitsindex. Ladeweg fuer eine Aufgabe: Router, Unterrouter, Detaildatei — an vier echten Fragen geprueft. **41 Dateien in 11 zusammengefuehrt**, rund 26'400 auf rund 14'600 Zeilen. Acht Dateien trugen einen `UEBERHOLT`-Kopf und standen trotzdem in der Navigation; darunter ein versandfertiger Mailtext, der Indien um seit 2026-08-05 gegenstandslose Pflege von 1'271 Artikeln gebeten haette — ersatzlos entfernt. Geloescht, weil ihre Aufgabe uebergegangen ist: `RAG_ROUTER.md`, `RAG_DETAIL_INDEX.md`, `MD_DOKUMENTENSTATUS_2026-05-20.md`. Gerettet statt geloescht: UK-Reimport-Falle und Legendenzeile nach `docs/rag/MANUAL_IMPORT.md`, Betriebsregeln des SAP-Reports `Z_TRAFAG_DACH_EXPORT` nach `docs/FINANCE_STANDARDKOSTEN.md`. NICHT angefasst: `docs/raw_md_archive/` bleibt unveraendert, `docs/FINANCE_UK2025_WERTFEHLER_2026-08-10.md` trug eine fremde nicht committete Aenderung, alle untracked Dateien. Pruefungen: jede der 95 `.md` genau einmal in `baum.md`, keine toten Verweise, keine Restveralterung ausser bewussten Warnzeilen. Nur Dokumentation, kein Anwendungscode, keine Datenbank, kein Deploy, keine Tests noetig. Reservierung frei |
...
|---|---|---|---|
| `lastchange.md`, `docs/rag/FINANCE.md`, `docs/rag/MANUAL_IMPORT.md`, `docs/Issue_Log_Ingo_2026-08-12.tsv` und `.xlsx`, `docs/Issue_Log_Konsolidiert_2026-08-12.tsv` und `.xlsx` | frei | 2026-08-17 | Abgeschlossen am 2026-08-17: Spanien-Buchungsdatum nachdokumentiert. Neuer Abschnitt in `lastchange.md`, Kurzstandzeile in `docs/rag/FINANCE.md`, korrigierter Kurzstand und korrigierte Skripthoheitstabelle in `docs/rag/MANUAL_IMPORT.md` (die Tabelle fuehrte `PostingDate` weiter als „nicht selektiert"). Zeile ISS-004.2 in beiden Issue-Logs auf `Laeuft` gesetzt, beide Arbeitsmappen mit `.tmp_tools/BuildIssueLogExcel` neu erzeugt; die TSV bleibt die fuehrende Fassung. `router.md` brauchte KEINE Aenderung, die Themenzeile fuer das spanische Buchungsdatum steht dort bereits. Nur Dokumentation. Reservierung frei |
...
| Produktivdeploy des gesamten aktuellen Workspace-Stands | frei | 2026-08-11 | Abgeschlossen und dokumentiert in `docs/DEPLOYMENT.md` |
...
`docs/Issue_Log_Konsolidiert_2026-08-12.tsv` (Punkt 3 fuehrte den Wertfehler als bereits
...
- Nachweise: `docs/FINANCE_SUPPLIER.md`, der vollständige
  Wiederaufnahmestand `docs/FINANCE_SUPPLIER.md` und das gestaltete,
