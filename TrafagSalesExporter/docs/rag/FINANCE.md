...
- OFFEN / bewusst nicht gefixt (Stand 2026-07-02): `Marge Original`/`Marge %` mischen weiterhin Waehrungen, wenn `StandardCostCurrency` von der Verkaufswaehrung abweicht (Pruefbuch, Gruppenmarge-Tab, Excel-Proof). Aktuell latent, weil `StandardCost` in den geprueften Daten ~0 ist und echte Konzern-Standardkosten (MBEW-STPRS) noch nicht angebunden sind. `Marge CHF` ist korrekt. Entscheidung offen: maskieren („-") vs. Kosten in Verkaufswaehrung umrechnen; am besten mit dem MBEW-STPRS-Feature umsetzen.
