...
- Andreas-Ruecklauf 2026-06-29 (a.docx Budget-CHF + b.docx Gruppenmarge ausgewertet). UMGESETZT (gruene/entschiedene Punkte): Interner-Lieferant-Erkennung auf genau drei Trafag-Gesellschaften eingegrenzt — Trafag AG (CH), Trafag Italy, Trafag India; alle anderen Lieferanten inkl. weiterer Intercompany-Gesellschaften (z. B. Magnetic Sense) zaehlen als 3rd Party. Logik zentral in `Services/GroupMarginSupplierClassifier.cs` (vorher dupliziert in `ManagementCockpitService` und `ExcelExportService`), mit Unit-Tests. Der frueher breite `"TRAFAG"`-Match (fing alle Gesellschaften als intern) ist entfernt. `117/117` Tests gruen.
- Andreas bestaetigt als bereits korrekt umgesetzt: externe Lieferanten nutzen Kosten aus der Verkaufszeile (Q2), interne Lieferkette nur eine Iteration (Q4), fehlende Standardkosten werden markiert statt geschaetzt, intern ungerundet mit 2-Dezimal-Anzeige.
- OFFEN / NICHT umgesetzt (gelb, brauchen Fachentscheid): (1) Gruppenmarge-Waehrung „CHF zum Jahreskurs" (b.docx Q5) — Kursquelle unklar (Budgetkurs vs. Ist-Jahreskurs) und bestehende Umrechnung laeuft pro Datum, nicht pro Jahr. (2) Gruppenmarge fehlende Standardpreise „Missing ODER pauschal 50%" (b.docx Q6) — eine Variante muss gewaehlt werden; aktuell bleibt es bei Missing. (3) Lieferanten-Erkennung langfristig ueber SAP-Stammdaten/eigene IC-Tabelle (Entity+Partnernummer, spaeter Magnetic Sense) statt Namensmatching (b.docx Q1, Haekchen A vs. Kommentar C widerspruechlich). (4) Budget-CHF: Andreas signalisiert Prioritaet „Group Currency / Reporting Cur" statt Budgetkurs (a.docx Q8/Q9); Kursfreigabe, Spaltenumfang, Owner und DE-2026-Umschaltung bleiben offen. Budget-CHF-Anzeigeort (a.docx Q6) daher noch nicht umgesetzt.
