...
internal sealed class HrKpiDashboardBuilder
{
        ...
        TurnoverPeriodScope period)
    {
        ...
        {
            new() { Label = "HC Basis", Value = FormatHeadcount(selectionHeadcount), Detail = period.ShowPeriodMetrics ? "Durchschnittlicher Headcount, nicht FTE" : "Aktueller Headcount, nicht FTE", Severity = "Normal", Theme = "Basis", HelpText = "Das ist die Personalzahl, durch die wir teilen, um die Fluktuation in Prozent auszurechnen. Gezaehlt werden alle Festangestellten als Koepfe - eine Person zaehlt als 1, egal ob Voll- oder Teilzeit. Es geht also NICHT um Stellenprozente. Je nachdem, was oben eingestellt ist, sehen Sie hier die heutige Personalzahl oder den Durchschnitt ueber den gewaehlten Zeitraum." },
            new() { Label = "Austritte total", Value = totalLeavers.ToString("N0"), Detail = "Alle Austritte in Auswahl", Severity = "Normal", Theme = "Leavers", HelpText = "Alle Austritte aus dem HR-System Rexx, die in die aktuelle Auswahl fallen. Hier sind noch saemtliche Gruende enthalten, also auch Pensionierungen, befristete Vertraege oder Kuendigungen durch Trafag. Diese Zahl ist deshalb noch nicht die Fluktuation - sie ist nur die Gesamtmenge aller Austritte." },
            new() { Label = "Austritte AN-Kuendigung", Value = employeeResignationCount.ToString("N0"), Detail = "Arbeitnehmer-/Mitarbeiterkuendigungen", Severity = "Normal", Theme = "Leavers", HelpText = "Austritte, bei denen die Mitarbeiterin oder der Mitarbeiter selbst gekuendigt hat (Arbeitnehmerkuendigung). Das ist ein Zwischenschritt: Aus diesen Austritten wird spaeter die Zahl gebildet, die fuer die Fluktuation zaehlt. Kuendigungen durch Trafag sind hier NICHT enthalten." },
            new() { Label = "Austritte relevant", Value = relevantLeaverCount.ToString("N0"), Detail = "Zaehlt fuer Fluktuation", Severity = "Normal", Theme = "Relevant", HelpText = "Das sind die Austritte, die wirklich fuer die Fluktuation zaehlen (nach der HR-Definition von Trafag, im Wesentlichen die Eigenkuendigungen). Jede Person wird nur einmal gezaehlt, auch wenn sie mehrfach in den Daten steht. Diese Zahl ist der obere Teil der Fluktuations-Rechnung, also die Zahl, die durch die Personalzahl geteilt wird." },
            new() { Label = "Austritte nicht relevant", Value = nonRelevantLeaverCount.ToString("N0"), Detail = "Ausgeschlossen oder unklar", Severity = nonRelevantLeaverCount > relevantLeaverCount ? "Warning" : "Normal", Theme = "Excluded", HelpText = "Austritte, die bewusst NICHT in die Fluktuation einfliessen, zum Beispiel Pensionierung, Praktikum, befristeter Vertrag, Kuendigung durch Trafag oder Faelle, bei denen der Grund nicht eindeutig ist. Wenn diese Zahl groesser ist als die relevanten Austritte, wird die Kachel zur Kontrolle gelb markiert - dann lohnt sich ein Blick in die Tabelle der Ausschlussgruende." },
            new() { Label = "Fluktuation Auswahl", Value = selectionRate.ToString("P1"), Detail = "Aktuelle Auswahl / HC, nicht annualisiert", Severity = selectionRate > 0.12m ? "Warning" : "Normal", Theme = "Rate", HelpText = "So wird gerechnet: die fuer die Fluktuation zaehlenden Austritte in der aktuellen Auswahl geteilt durch die Personalzahl (Kachel 'HC Basis'), dann als Prozent. Dieser Wert ist NICHT auf ein ganzes Jahr hochgerechnet, sondern zeigt genau die aktuelle Auswahl (zum Beispiel eine Abteilung oder einen bestimmten Zeitraum)." },
            new() { Label = "Ausschlussgruende", Value = totalLeavers.ToString("N0"), Detail = "Basis fuer Ausschluss-Tabelle", Severity = "Normal", Theme = "Excluded", HelpText = "Grundlage fuer die Tabelle der Ausschlussgruende weiter unten. Diese Tabelle zeigt Ihnen fuer jeden Austritt, aus welchem Grund er nicht zur Fluktuation gezaehlt wird (zum Beispiel Pensionierung oder befristeter Vertrag)." }
        };
        ...
        metrics[0] = new HrKpiMetric
        {
            ...
            Severity = "Normal",
            Theme = "Basis",
            HelpText = $"Die Personalzahl, durch die wir fuer den Zeitraum seit Jahresbeginn teilen. Es ist der Durchschnitt aller Festangestellten vom {FormatDateShort(yearStart)} bis {FormatDateShort(yearEnd)}. Gezaehlt werden Koepfe (eine Person = 1), nicht Stellenprozente."
        };

        metrics.AddRange(
        [
            new() { Label = "HC Monat", Value = FormatHeadcount(monthHeadcount), Detail = $"Avg HC {currentMonth:N0}/{year}, nicht FTE", Severity = "Normal", Theme = "Basis", HelpText = $"Die durchschnittliche Personalzahl im Monat {currentMonth:N0}/{year}. Sie wird gebildet aus dem Personalstand am Monatsanfang plus dem am Monatsende, geteilt durch 2. Gezaehlt werden Koepfe, nicht Stellenprozente. Diese Zahl ist die Basis, durch die bei der Monats-Fluktuation geteilt wird." },
            new() { Label = "Fluktuation Monat", Value = monthRate.ToString("P1"), Detail = $"{monthLeaverCount:N0} relevante Austritte / HC {FormatHeadcount(monthHeadcount)}", Severity = monthRate > 0.03m ? "Warning" : "Normal", Theme = "Rate", HelpText = $"So wird gerechnet: die fuer die Fluktuation zaehlenden Austritte im Monat {currentMonth:N0}/{year} geteilt durch die durchschnittliche Personalzahl dieses Monats, dann als Prozent. Der Wert gilt nur fuer diesen einen Monat und ist NICHT auf ein ganzes Jahr hochgerechnet." },
            new() { Label = "HC Quartal", Value = FormatHeadcount(quarterHeadcount), Detail = $"Avg HC Q{currentQuarter}/{year}, nicht FTE", Severity = "Normal", Theme = "Basis", HelpText = $"Die durchschnittliche Personalzahl im Quartal Q{currentQuarter}/{year} (Mittelwert der drei Monatswerte). Gezaehlt werden Koepfe, nicht Stellenprozente. Diese Zahl ist die Basis, durch die bei der Quartals-Fluktuation geteilt wird." },
